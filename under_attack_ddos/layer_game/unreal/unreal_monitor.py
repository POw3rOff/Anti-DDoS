#!/usr/bin/env python3
"""
Unreal Engine (Ark/Palworld) Protocol Monitor
Part of Layer G (Game Protocol Defense)

Responsibility: Initial support for Unreal Engine 4/5 protocol anomalies.
Protocol: Unreal (UDP).
"""
...
# --- Identity ---
SCRIPT_NAME = "unreal_monitor"
GAME_NAME = "UnrealEngine"

class UnrealMonitor(GameProtocolParser):
    def __init__(self, config_path=None, interface=None, port=7777, dry_run=False):
        ...
        self.max_pps = self.config.get("max_pps_per_ip", 100)

    def packet_callback(self, packet):
        """Processes UDP traffic on Unreal port."""
    ...
    def analyze(self):
        ...
        for ip, count in list(self.packet_counts.items()):
            pps = count / duration
            if pps > self.max_pps:
                self.emit_event("unreal_flood", ip, "MEDIUM", {
                    "pps": round(pps, 1),
    ...
    def run(self):
        ...
        logging.info(f"Monitoring Unreal Engine on {bpf_filter}...")
...
def main():
    parser = argparse.ArgumentParser(description="Unreal Engine Protocol Monitor")
    parser.add_argument("--config", help="Path to config.yaml")
    parser.add_argument("--interface", help="Interface to sniff")
    parser.add_argument("--port", type=int, default=7777, help="Unreal Port (Default: 7777)")
    ...
    monitor = UnrealMonitor(args.config, args.interface, args.port, args.dry_run)
    monitor.run()
